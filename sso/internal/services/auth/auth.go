...
type Auth struct { // Связывание: Вот тут ключевое звено. Хотя структура Auth сама по себе не реализует интерфейс напрямую, но если мы реализуем в структуре Auth все методы, требуемые интерфейсом, тогда она автоматически становится совместимой с этим интерфейсом. То есть даже если мы не прописываем явно "реализует интерфейс X", Go-компилятор поймёт, что эта структура подходит для интерфейса, поскольку соответствует требованиям по методам.
	log          *slog.Logger
	userSaver    UserSaver
...
// интерфейсы с сигнатурой длдя работы с БД
